...
int main_fft3()
{
	...
	int bf_rnd_stg2 = 0;
	int bf_rnd_stg3 = 0;
	int bi_rnd_stg0 = 30 - bf_rnd_stg0;
	int bi_rnd_stg1 = 30 - bf_rnd_stg1;
	int bi_rnd_stg2 = 30 - bf_rnd_stg2;
	int bi_rnd_stg3 = 30 - bf_rnd_stg3;
	...
	bam s3m0 (Nt, Nia_s3m0, Nia_s3m0,  Nia_s3m0-1,  0, 1); // never touch this
	bam s3m1 (Nt, Nia_s3m1, Nia_s3m1,  Nia_s3m1-1,  0, 1); // never touch this
	bam s3m2 (Nt, Nia_s3m2, Nia_s3m2,  Nia_s3m2-1,  0, 1); // never touch this
	bam s3m3 (Nt, Nia_s3m3, Nia_s3m3,  Nia_s3m3-1,  0, 1); // never touch this
	loa s3o0 (Nt, Nia_s3o0, Nia_s3o0-1, 0, 1); // never touch this
...
//		fft256_fixed(data_fix2, N, &bi[0], &bf[0]);
		...
		for (n2=0; n2<N2; n2++)
		{
			...
			for (k1=0; k1<N1; k1++)
			{
				...
				p0 = (bfly[k1].r*W.r); p0 = (int)float2fix((float)p0 / pow(2.0, bf_fft_stg0), bi_rnd_stg0, bf_rnd_stg0);
				p1 = (-bfly[k1].i*W.i); p1 = (int)float2fix((float)p1 / pow(2.0, bf_fft_stg0), bi_rnd_stg0, bf_rnd_stg0);
				p2 = (bfly[k1].i*W.r); p2 = (int)float2fix((float)p2 / pow(2.0, bf_fft_stg0), bi_rnd_stg0, bf_rnd_stg0);
				p3 = (bfly[k1].r*W.i); p3 = (int)float2fix((float)p3 / pow(2.0, bf_fft_stg0), bi_rnd_stg0, bf_rnd_stg0);
		...
		if (mode == 2 || mode == 4 || mode == 6 || mode == 8) {
			...
			for (n2=0; n2<N2; n2++)
			{
				...
				for (k1=0; k1<N1; k1++)
				{
					...
					p0 = (s0m0.calc(bfly[k1].r, W.r)); p0 = (int)float2fix((float)p0 / pow(2.0, bf_fft_stg0), bi_rnd_stg0, bf_rnd_stg0);
					p1 = (s0m1.calc(-bfly[k1].i, W.i)); p1 = (int)float2fix((float)p1 / pow(2.0, bf_fft_stg0), bi_rnd_stg0, bf_rnd_stg0);
					p2 = (s0m2.calc(bfly[k1].i, W.r)); p2 = (int)float2fix((float)p2 / pow(2.0, bf_fft_stg0), bi_rnd_stg0, bf_rnd_stg0);
					p3 = (s0m3.calc(bfly[k1].r, W.i)); p3 = (int)float2fix((float)p3 / pow(2.0, bf_fft_stg0), bi_rnd_stg0, bf_rnd_stg0);
		...
		for (int k2=0; k2<R2; k2++)
		{
			...
			for (n2=0; n2<N2; n2++)
			{
				...
				for (k1=0; k1<N1; k1++)
				{
					...
					p0 = (bfly[k1].r*W.r); p0 = (int)float2fix((float)p0 / pow(2.0, bf_fft_stg1), bi_rnd_stg1, bf_rnd_stg1);
					p1 = (-bfly[k1].i*W.i); p1 = (int)float2fix((float)p1 / pow(2.0, bf_fft_stg1), bi_rnd_stg1, bf_rnd_stg1);
					p2 = (bfly[k1].i*W.r); p2 = (int)float2fix((float)p2 / pow(2.0, bf_fft_stg1), bi_rnd_stg1, bf_rnd_stg1);
					p3 = (bfly[k1].r*W.i); p3 = (int)float2fix((float)p3 / pow(2.0, bf_fft_stg1), bi_rnd_stg1, bf_rnd_stg1);
			...
			for (int k2=0; k2<R2; k2++)
			{
				...
				for (n2=0; n2<N2; n2++)
				{
					...
					for (k1=0; k1<N1; k1++)
					{
						...
						p0 = (s1m0.calc(bfly[k1].r, W.r)); p0 = (int)float2fix((float)p0 / pow(2.0, bf_fft_stg1), bi_rnd_stg1, bf_rnd_stg1);
						p1 = (s1m1.calc(-bfly[k1].i, W.i)); p1 = (int)float2fix((float)p1 / pow(2.0, bf_fft_stg1), bi_rnd_stg1, bf_rnd_stg1);
						p2 = (s1m2.calc(bfly[k1].i, W.r)); p2 = (int)float2fix((float)p2 / pow(2.0, bf_fft_stg1), bi_rnd_stg1, bf_rnd_stg1);
						p3 = (s1m3.calc(bfly[k1].r, W.i)); p3 = (int)float2fix((float)p3 / pow(2.0, bf_fft_stg1), bi_rnd_stg1, bf_rnd_stg1);
		...
		for (int k2=0; k2<R3; k2++)
		{
			...
			for (n2=0; n2<N2; n2++)
			{
				...
				for (k1=0; k1<N1; k1++)
				{
					...
					p0 = (bfly[k1].r*W.r); p0 = (int)float2fix((float)p0 / pow(2.0, bf_fft_stg2), bi_rnd_stg2, bf_rnd_stg2);
					p1 = (-bfly[k1].i*W.i); p1 = (int)float2fix((float)p1 / pow(2.0, bf_fft_stg2), bi_rnd_stg2, bf_rnd_stg2);
					p2 = (bfly[k1].i*W.r); p2 = (int)float2fix((float)p2 / pow(2.0, bf_fft_stg2), bi_rnd_stg2, bf_rnd_stg2);
					p3 = (bfly[k1].r*W.i); p3 = (int)float2fix((float)p3 / pow(2.0, bf_fft_stg2), bi_rnd_stg2, bf_rnd_stg2);
			...
			for (int k2=0; k2<R3; k2++)
			{
				...
				for (n2=0; n2<N2; n2++)
				{
					...
					for (k1=0; k1<N1; k1++)
					{
						...
						p0 = (s2m0.calc(bfly[k1].r, W.r)); p0 = (int)float2fix((float)p0 / pow(2.0, bf_fft_stg2), bi_rnd_stg2, bf_rnd_stg2);
						p1 = (s2m1.calc(-bfly[k1].i, W.i)); p1 = (int)float2fix((float)p1 / pow(2.0, bf_fft_stg2), bi_rnd_stg2, bf_rnd_stg2);
						p2 = (s2m2.calc(bfly[k1].i, W.r)); p2 = (int)float2fix((float)p2 / pow(2.0, bf_fft_stg2), bi_rnd_stg2, bf_rnd_stg2);
						p3 = (s2m3.calc(bfly[k1].r, W.i)); p3 = (int)float2fix((float)p3 / pow(2.0, bf_fft_stg2), bi_rnd_stg2, bf_rnd_stg2);
		...
		for (int k2=0; k2<R4; k2++)
		{
			...
			for (n2=0; n2<N2; n2++)
			{
				...
				for (k1=0; k1<N1; k1++)
				{
					...
					p0 = (bfly[k1].r*W.r); p0 = (int)float2fix((float)p0 / pow(2.0, bf_fft_stg3), bi_rnd_stg3, bf_rnd_stg3);
					p1 = (-bfly[k1].i*W.i); p1 = (int)float2fix((float)p1 / pow(2.0, bf_fft_stg3), bi_rnd_stg3, bf_rnd_stg3);
					p2 = (bfly[k1].i*W.r); p2 = (int)float2fix((float)p2 / pow(2.0, bf_fft_stg3), bi_rnd_stg3, bf_rnd_stg3);
					p3 = (bfly[k1].r*W.i); p3 = (int)float2fix((float)p3 / pow(2.0, bf_fft_stg3), bi_rnd_stg3, bf_rnd_stg3);
...
//						p3 = (s3m3.calc(bfly[k1].r, W.i))>>bf_fft_stg3;
						...
						p0 = (s3m0.calc(bfly[k1].r, W.r)); p0 = (int)float2fix((float)p0 / pow(2.0, bf_fft_stg3), bi_rnd_stg3, bf_rnd_stg3);
						p1 = (s3m1.calc(-bfly[k1].i, W.i)); p1 = (int)float2fix((float)p1 / pow(2.0, bf_fft_stg3), bi_rnd_stg3, bf_rnd_stg3);
						p2 = (s3m2.calc(bfly[k1].i, W.r)); p2 = (int)float2fix((float)p2 / pow(2.0, bf_fft_stg3), bi_rnd_stg3, bf_rnd_stg3);
						p3 = (s3m3.calc(bfly[k1].r, W.i)); p3 = (int)float2fix((float)p3 / pow(2.0, bf_fft_stg3), bi_rnd_stg3, bf_rnd_stg3);
		...
		/*
		for (int s=0;s<N;s++) {
			cout << "Fixed real(" << s << "): " << data_fix[s].r << ", Floating real: " << data_float[s].r << endl;
			cout << "Fixed imag(" << s << "): " << data_fix[s].i << ", Floating imag: " << data_float[s].i << endl;
		}
		*/
...
//			err_pwr += (data_fix[s].r-data_fix2[s].r)*(data_fix[s].r-data_fix2[s].r)+(data_fix[s].i-data_fix2[s].i)*(data_fix[s].i-data_fix2[s].i);
			sig_pwr += data_float[s].r*data_float[s].r + data_float[s].i*data_float[s].i;
			err_pwr += (data_fix2[s].r - data_float[s].r)*(data_fix2[s].r - data_float[s].r) + (data_fix2[s].i - data_float[s].i)*(data_fix2[s].i - data_float[s].i);

//			cout << "sglee acc imag: " << data_fix[s].i << ", inacc imag: " << data_fix2[s].i << endl;

		}
		//cout << "sglee sig_pwr " << sig_pwr/N << endl;
	...
	if (mode == 2 || mode == 4 || mode == 6 || mode == 8) {
		...
		double errs2t0[8] = {0, 0.43, 0.97, 1.98, 3.98, 8.01, 16.04, 28.23};
		double errs2t1[8] = {0, 0.43, 0.97, 1.99, 4.00, 7.98, 16.00, 32.2};
		double errs2t2[8] = {0, 2453.67, 6329.1, 13963.4, 28854.9, 57598.3, 110555, 191149};
		double errs2t3[8] = {0, 1615.27, 4135.73, 9103.33, 18791.6, 37376.9, 71804.2, 123493};
		double errs2t4[8] = {0, 2455.04, 6321.69, 13948.5, 28816.7, 57468.2, 110005, 188721};
		double errs2t5[8] = {0, 1614.4, 4140.41, 9112.76, 18815.9, 37559.7, 72155.6, 125059};
		double errs2t6[8] = {0, 0.34, 0.75, 1.54, 3.1, 6.22, 12.5, 17.4};

		double errs3t0[8] = {0, 0.43, 0.97, 1.98, 4.00, 8.00, 16.00, 28.17};
		double errs3t1[8] = {0, 0.43, 0.97, 1.99, 4.00, 7.98, 16.00, 32.1};
		...
		double snr_est = (var_x1+var_x2)/(evars0r+evars0i+evars1r+evars1i+evars2r+evars2i+evars3r+evars3i+4/3);
	...
	for (int Nia_add_s2i0 = 1; Nia_add_s2i0 < 8; Nia_add_s2i0++) {
		loa add_s2i0 (Nt, Nia_add_s2i0, Nia_add_s2i0-1, 0, 1);
	...
	for (int Nia_add_s2i1 = 1; Nia_add_s2i1 < 8; Nia_add_s2i1++) {
		loa add_s2i1 (Nt, Nia_add_s2i1, Nia_add_s2i1-1, 0, 1);
	...
	for (int Nia_mul_s2t0 = 1; Nia_mul_s2t0 < 8; Nia_mul_s2t0++) {
		bam mul_s2t0 (2*Nt, Nia_mul_s2t0, Nia_mul_s2t0, Nia_mul_s2t0-1, 0, 1);
	...
	for (int Nia_mul_s2t1 = 1; Nia_mul_s2t1 < 8; Nia_mul_s2t1++) {
		bam mul_s2t1 (2*Nt, Nia_mul_s2t1, Nia_mul_s2t1, Nia_mul_s2t1-1, 0, 1);
	...
	for (int Nia_mul_s2t2 = 1; Nia_mul_s2t2 < 8; Nia_mul_s2t2++) {
		bam mul_s2t2 (2*Nt, Nia_mul_s2t2, Nia_mul_s2t2, Nia_mul_s2t2-1, 0, 1);
	...
	for (int Nia_mul_s2t3 = 1; Nia_mul_s2t3 < 8; Nia_mul_s2t3++) {
		bam mul_s2t3 (2*Nt, Nia_mul_s2t3, Nia_mul_s2t3, Nia_mul_s2t3-1, 0, 1);
	...
	for (int Nia_add_s2o0 = 1; Nia_add_s2o0 < 8; Nia_add_s2o0++) {
		loa add_s2o0 (Nt, Nia_add_s2o0, Nia_add_s2o0-1, 0, 1);
	...
	for (int Nia_add_s2o1 = 1; Nia_add_s2o1 < 8; Nia_add_s2o1++) {
		loa add_s2o1 (Nt, Nia_add_s2o1, Nia_add_s2o1-1, 0, 1);
	...
	if (mode == 7) {
		...
		for (int Nia_add_s3i0 = 1; Nia_add_s3i0 < 8; Nia_add_s3i0++) {
			loa add_s3i0 (Nt, Nia_add_s3i0, Nia_add_s3i0-1, 0, 1);
		...
		for (int Nia_add_s3i1 = 1; Nia_add_s3i1 < 8; Nia_add_s3i1++) {
			loa add_s3i1 (Nt, Nia_add_s3i1, Nia_add_s3i1-1, 0, 1);
		...
		for (int Nia_mul_s3t0 = 1; Nia_mul_s3t0 < 8; Nia_mul_s3t0++) {
			bam mul_s3t0 (2*Nt, Nia_mul_s3t0, Nia_mul_s3t0, Nia_mul_s3t0-1, 0, 1);
		...
		for (int Nia_mul_s3t1 = 1; Nia_mul_s3t1 < 8; Nia_mul_s3t1++) {
			bam mul_s3t1 (2*Nt, Nia_mul_s3t1, Nia_mul_s3t1, Nia_mul_s3t1-1, 0, 1);
		...
		for (int Nia_mul_s3t2 = 1; Nia_mul_s3t2 < 8; Nia_mul_s3t2++) {
			bam mul_s3t2 (2*Nt, Nia_mul_s3t2, Nia_mul_s3t2, Nia_mul_s3t2-1, 0, 1);
		...
		for (int Nia_mul_s3t3 = 1; Nia_mul_s3t3 < 8; Nia_mul_s3t3++) {
			bam mul_s3t3 (2*Nt, Nia_mul_s3t3, Nia_mul_s3t3, Nia_mul_s3t3-1, 0, 1);
		...
		for (int Nia_add_s3o0 = 1; Nia_add_s3o0 < 8; Nia_add_s3o0++) {
			loa add_s3o0 (Nt, Nia_add_s3o0, Nia_add_s3o0-1, 0, 1);
		...
		for (int Nia_add_s3o1 = 1; Nia_add_s3o1 < 8; Nia_add_s3o1++) {
			loa add_s3o1 (Nt, Nia_add_s3o1, Nia_add_s3o1-1, 0, 1);
