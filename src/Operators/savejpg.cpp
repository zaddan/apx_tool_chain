...
static SWORD DU_DCT_sglee[64]; // Current DU (after DCT and quantization) which we'll zigzag
static SWORD DU[64]; //zigzag reordered DU which will be Huffman coded

// sglee
float fd_data[64];
float fd_data_fixed[64];
SBYTE td_data[64];
SBYTE td_data_fixed[64];
double quan_noise[64];
...
// Set quantization table and zigzag reorder it
{
 ...
 for (i = 0; i < 64; i++) {
    ...
    //printf ("SGLEE TBL GEN: %d => %d, INDEX: %d => %d \n",basic_table[i], temp, i, zigzag[i]);
    //printf ("SGLEE TBL GEN2: %d \n", 100L);
...
void set_DQTinfo()
{
 // sglee to global variable BYTE scalefactor=50;// scalefactor controls the visual quality of the image
   ...
   for (col = 0; col < 8; col++)
     {
       ...
       fdtbl_Cb[i] = (float) (1.0 / ((double) DQTinfo.Cbtable[zigzag[i]] *
			  aanscalefactor[row] * aanscalefactor[col] * 8.0));
       // printf ("SGLEE QTBL : %f, %f \n", fdtbl_Y[i], fdtbl_Cb[i]);
...
// Quantize/descale the coefficients, and store into output array
 ...
 // Apply the quantization and scaling factor
   ...
   //code should work for either 16-bit or 32-bit ints.
     ...
     //printf ("SGLEE DCT: %f, FDTBL: %f at IDX: %d = %f with INT: %d \n", datafloat[i], fdtbl[i], i, temp, outdata[i]);

     // printf ("SGLEE QUAN: %f => %f, %d \n", temp, (float)outdata[i], dezigzag[i]);
     quan_noise[i] += (temp - (float)outdata[i])*(temp - (float)outdata[i]);
...
void DCT(SBYTE *f,float *F)
{
  ...
  for(int u=0;u<8;u++) {
    for(int v=0;v<8;v++) {
	    ...
	    for(int x=0;x<8;x++) {
	      for(int y=0;y<8;y++) {
	        a += float(f[8*x+y])*cos((2.0*float(x)+1.0)*float(u)*3.14/16.0)*cos((2.0*float(y)+1.0)*float(v)*3.14/16.0);
        }
      }
	    F[8*u+v] = float(0.25*C2(u)*C2(v)*a);
    }
  }
}

void DCTD(float *f, SBYTE *F)
{
  float a;
  for(int x=0;x<8;x++)
    for(int y=0;y<8;y++) {
     ...
     for(int u=0;u<8;u++)
       for(int v=0;v<8;v++) {
         a += C2(u)*C2(v)*float(f[8*u+v])*cos((2.0*float(x)+1.0)*float(u)*3.14/16.0)*cos((2.0*float(y)+1.0)*float(v)*3.14/16.0);
       }
     F[8*x+y] = (SBYTE)(0.25*a);
   }
}

void quantization_fixed2(float *data, SWORD *outdata) {
  float temp;
  int q[64] = {16,11,10,16,24,40,51,61,
    ...
    long temp2;
    for (i = 0; i < 64; i++) {
      temp2 = ((long) q[i] * scalefactor + 50L) / 100L;
		  //limit the values to the valid range
      if (temp2 <= 0L) temp2 = 1L;
      if (temp2 > 255L) temp2 = 255L; //limit to baseline range if requested
      q[i] = (BYTE) temp2;
      // printf ("SGLEE TBL GEN: %d => %d, INDEX: %d => %d \n",basic_table[i], temp, i, zigzag[i]);
      // printf ("SGLEE TBL GEN2: %d \n", 100L);
  ...
  for (int i = 0; i < 64; i++) {
    // Apply the quantization and scaling factor
    // temp = data[i]/(float)q[zigzag[i]];
    temp = data[i]/(float)q[i];

    //printf ("SGLEE QUAN : %f data[i], FDTBL: %f at IDX: %d = %f \n", data[i], (float) q[i], i, temp);
		...
		bitstring *HTDC,bitstring *HTAC)
{
 ...
 // fdct_and_quantization_fixed(ComponentDU,fdtbl,DU_DCT_sglee);

 // fdct_fixed(ComponentDU,fd_data);
 // floating point
 DCT(ComponentDU,fd_data);
 // sglee DCTD(fd_data,td_data);

 // fixed point
 // sglee DCT_fixed (ComponentDU,fd_data_fixed, blk_pwr);
 // sglee DCTD(fd_data_fixed,td_data_fixed);

 for (int k=0;k<64;k++) {
   // printf ("SGLEE: DCT1 %d, DCT2 %d %d \n", td_data[k], td_data_fixed[k], k);
 }

 // sglee quantization_fixed(fd_data,DU_DCT);
 quantization_fixed2(fd_data,DU_DCT);
...
void main_encoder()
{
 ...
 for (ypos=0;ypos<Yimage;ypos+=8) {
  ...
  // sglee double psnr_cal = 10 * log10( 255.0*255.0 / (td_mse_sum/(float)k));
