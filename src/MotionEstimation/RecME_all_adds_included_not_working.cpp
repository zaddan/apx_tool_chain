...
//        pbIn1 = myOp[1]->calc_ref(pbInput1, y*iStride0);
		pbIn0 = (byte*)myOp[0]->calc(pbInput0 , y*iStride0);
		pbIn1 = (byte*)myOp[1]->(pbInput1 , y*iStride0);
	...
	i32 iPelsAvailLeft, i32 iPelsAvailRight, i32 iPelsAvailTop, i32 iPelsAvailBottom)
{
	...
	for(i32 i=0;i<m_iImgHeightInBlks;i++,x=0,y+=m_iBlkSize)
	{
		for(i32 j=0;j<m_iImgWidthInBlks;j++,iBlkNum++,x+=m_iBlkSize)
		{
            ...
            m_piSAD[iBlkNum] = GetMinSADPt(&pbCurrFrame[myOp[24]->calc(y*m_iImgStride,x)],m_iImgStride,m_iBlkSize, &pbRefFrame[myOp[25]->calc(iSWMinY*m_iImgStride,iSWMinX)],
				iSWWidth,iSWHeight,m_iImgStride,iMinPtX,iMinPtY);

			// Generate the MV, from the current frame to the reference frame
			m_ppbMVRightToLeft[0][iBlkNum] = myOp[26]->calc(myOp[27]->calc(iSWMinX,iMinPtX),-1*x);
			m_ppbMVRightToLeft[1][iBlkNum] = myOp[28]->calc(myOp[29]->calc(iSWMinY,iMinPtY),-1*y);
		}
	}
}
...
{
	i32 iBlkIdx = myOp[]->calc(iY/m_iBlkSize*m_iImgWidthInBlks, iX/m_iBlkSize);
	iMVX = m_ppbMVRightToLeft[0][iBlkIdx];
	iMVY = m_ppbMVRightToLeft[1][iBlkIdx];
}
