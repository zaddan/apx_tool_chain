...
BYTE bytenew=0; // The byte that will be written in the JPG file
SBYTE bytepos=7; //bit position in the byte we write (bytenew)
			//should be<=7 and >=0
...
BYTE scalefactor=10;// scalefactor controls the visual quality of the image
...
void set_DQTinfo()
{
 ...
 DQTinfo.QTYinfo=0;
 DQTinfo.QTCbinfo=1;
 cout <<"this is the scale factoer"<< (int) scalefactor<<endl;
...
//-----------------------------------------------------------------------------------------------------------------------------
////-----------------------------------------------------------------------------------------------------------------------------
////---------module_name::: fdct_and_quantization
////---------functionlity::: this function takes care of two encoding stages, meaning fdct and quantization
////-----------------------------------------------------------------------------------------------------------------------------
////-----------------------------------------------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------------------------------------------
////-----------------------------------------------------------------------------------------------------------------------------
////---------module_name::: process_DU
////---------functionlity::: This module takes care of fdct + quantization + zigzag reorder + difference encoding
////-----------------------------------------------------------------------------------------------------------------------------
////-----------------------------------------------------------------------------------------------------------------------------
void process_DU(SBYTE *ComponentDU,float *fdtbl,SWORD *DC,
		bitstring *HTDC,bitstring *HTAC, string outputFile1Name, string outputFile2Name, string outputFileName3)
{
 ...
 //fdct  
 fdct(ComponentDU, fdtbl, datafloat);
 writeOutput(outputFile1Name.c_str(), datafloat, 64, "start", "end");
 //quantization 
 quantization(fdtbl,DU_DCT, datafloat);
 writeOutput(outputFile2Name.c_str(), DU_DCT, 64, "start", "end");
 //erase alter 
 /*for (int i =0; i < 64;  i++) {*/
     //cout << "ther" << DU_DCT[i] <<"\n";
 /*}*/
 ...
 //hufman coding 
 huffmanCoding(DC, HTDC,HTAC, EOB, M16zeroes);
...
//-----------------------------------------------------------------------------------------------------------------------------
////-----------------------------------------------------------------------------------------------------------------------------
////---------module_name::: load_data_units_from_RGB_buffer
////---------functionlity::: this module converst the RGB picture to the Y, Cb, Cr format
////-----------------------------------------------------------------------------------------------------------------------------
////-----------------------------------------------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------------------------------------------
////-----------------------------------------------------------------------------------------------------------------------------
////---------module_name::: main_encoder
////---------functionlity::: this is the main function for the encoder of jpeg
////-----------------------------------------------------------------------------------------------------------------------------
////-----------------------------------------------------------------------------------------------------------------------------
void main_encoder()
{
 ...
 rmOutputFile("fdct_results.out");
 rmOutputFile("quantization_results.out");
 rmOutputFile("huffman_coding_results.out");
 for (ypos=0;ypos<Yimage;ypos+=8)
  for (xpos=0;xpos<Ximage;xpos+=8)
   {
    //converting the RGB format to the YCb 
    //I am not applying any apx technique to this part  
    load_data_units_from_RGB_buffer(xpos,ypos);
    //fdct + quantization + zigzag reorder + Huffman encoding (I believe we extract the difference
    //and then do the coding)
    process_DU(YDU,fdtbl_Y,&DCY,YDC_HT,YAC_HT,"fdct_results.out", "quantization_results.out", "huffman_coding_results.out");
    process_DU(CbDU,fdtbl_Cb,&DCCb,CbDC_HT,CbAC_HT,"fdct_results.out", "quantization_results.out", "huffman_coding_results.out");
    process_DU(CrDU,fdtbl_Cb,&DCCr,CbDC_HT,CbAC_HT,"fdct_results.out", "quantization_results.out", "huffman_coding_results.out");
   }
}
