...
int func(int x, int y, const int block[8][8])
{
    ...
    for( int u=0; u<8; u++)
    {
         for(int v=0; v<8; v++)
         {
             float temp1 =  (float)myOp[51]->calc(C(u), C(v)); //MultiplicationOp
             float temp2 =  (float) (myOp[52]->calc(block[u][v], cosf(((2*x+1) * u * myPI) / 16))); //MultiplicationOp
             float temp3 = (float) myOp[53]->calc(temp1, temp2); //MultiplicationOp   
             float temp4 = (float) myOp[54]->calc(temp3, cosf( ((2*y+1) * v * myPI) / 16)); //MultiplicationOp
...
void DequantizeBlock( int block[64], const float quantBlock[64] )
{
    ...
    for( int c=0; c<64; c++)
	{
        block[c] = (int)(myOp[55]->calc(block[c], quantBlock[c])); //MultiplicationOp
    }
}
...
//	DumpHufCodes(c->m_acTable);
	...
	if ( jdata->m_stream[0]==0xff && jdata->m_stream[1]!=0x00)
	{
		// Something might be wrong, we should have had an interval marker set
         ...
         /*cout<< "indx needs to be between 0 and 4"<<end;*/
         //exit(1);
        /*}*/
        
    
        assert(jdata->m_restart_interval>0);
...
inline void DecodeMCU(stJpegData *jdata, int w, int h)
{
	...
	for (int y=0; y<h; y++)
	{
		for (int x=0; x<w; x++)
		{
			...
			assert(cY>0 && cY<COMPONENTS);
			assert(offset>=0 && offset<64*4);
