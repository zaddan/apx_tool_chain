...
int func(int x, int y, const int block[8][8])
{
    ...
    for( int u=0; u<8; u++)
    {
         for(int v=0; v<8; v++)
         {
             float temp1 =  (float)(C(u) *  C(v); //MultiplicationOp
             float temp2 =  (float) (block[u][v] * (cosf(((2*x+1) * u * myPI) / 16))); //MultiplicationOp
             float temp3 = (float) temp1 * temp2; //MultiplicationOp   
             float temp4 = (float) (temp3* (cosf( ((2*y+1) * v * myPI) / 16)); //MultiplicationOp
...
void DequantizeBlock( int block[64], const float quantBlock[64] )
{
    ...
    for( int c=0; c<64; c++)
	{
        block[c] = (int)((block[c] * quantBlock[c])); //MultiplicationOp
    }
}
